function [total_boxes points] = detect_face_on_image(img,minsize,PNet,RNet,ONet,threshold,fastresize,factor,numpoints,pointoutputs,extensionfactor)
	%im: input image
	...
	if numbox>0
		...
		if numbox>0
			...
			tempimg=(tempimg-127.5)*0.0078125; % 1/128 = 0.0078125
            ...
            points(1:numpoints,:)=repmat(w',[numpoints 1]).*points(1:numpoints,:)+repmat(total_boxes(:,1)',[numpoints 1])-1;
            points(numpoints+1:numpoints*2,:)=repmat(h',[numpoints 1]).*points(numpoints+1:numpoints*2,:)+repmat(total_boxes(:,2)',[numpoints 1])-1;
			if size(total_boxes,1)>0				
